...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        dapd_return_attn_scores: Optional[bool] = False,
    ) -> torch.Tensor:
        """
        Computes scaled dot product attention on query, key and value tensors, using an optional
        attention mask if passed, and applying dropout if a probability greater than 0.0 is specified.
        """
        needs_attn_scores = return_attn_scores or dapd_return_attn_scores
        if self.flash_attn_func is not None and attn_mask is None and not needs_attn_scores:
            ...
            dapd_keep_layers = min(4, self.config.n_layers)
            dapd_layer_start = self.config.n_layers - dapd_keep_layers
            if dapd_return_attn_scores and self.layer_id >= dapd_layer_start:
                avg_attn_scores = attn_avg(q, k, attn_mask).clone()
            elif return_attn_scores and self.layer_id >= 28:
                avg_attn_scores = attn_avg(q, k, attn_mask).clone()
    ...
    def attention(
        ...
        att, avg_attn_scores = self._scaled_dot_product_attention(
            ...
            k,
            v,
            attn_mask=attention_bias if dapd_return_attn_scores else None,
            dropout_p=0.0 if not self.training else self.config.attention_dropout,
            is_causal=False,
            return_attn_scores=return_attn_scores,
            dapd_return_attn_scores=dapd_return_attn_scores,
        )
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache, avg_attn_scores = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias=attention_bias if dapd_return_attn_scores else None, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position,return_attn_scores=return_attn_scores,dapd_return_attn_scores=dapd_return_attn_scores
            )
        else:
            if dapd_return_attn_scores:
                att, cache, avg_attn_scores = self.attention(q, k, v, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position,return_attn_scores=return_attn_scores,dapd_return_attn_scores=dapd_return_attn_scores)
            else:
...
class LLaDAModel(nn.Module):
    ...
    def forward(
            ...
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    x, cache, avg_attn_scores = self._activation_checkpoint_fn(
                        block, x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position, return_attn_scores=return_attn_scores,dapd_return_attn_scores=dapd_return_attn_scores
                    )
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, avg_attn_scores = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position, return_attn_scores=return_attn_scores,dapd_return_attn_scores=dapd_return_attn_scores)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
                ...
                elif dapd_return_attn_scores and block_idx >= dapd_layer_start:
                    if avg_attn_scores is None:
                        raise RuntimeError("DAPD expected attention scores from a top layer, but got None.")
                    all_avg_scores.append(avg_attn_scores)
        else:
        ...
        if return_attn_scores or dapd_return_attn_scores:
            if len(all_avg_scores) == 0:
                raise RuntimeError("Attention score collection was requested, but no layers were collected.")
