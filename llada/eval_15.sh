...
CUDA_VISIBLE_DEVICES=${device} accelerate launch eval_llada.py --tasks ${task} --num_fewshot ${num_fewshot} \
--confirm_run_unsafe_code --model llada_dist \
--model_args model_path=${model_path},gen_length=${length},steps=${steps},block_length=${block_length},show_speed=True,dapd=True,dapd_single_block=False,dapd_tau_min=0.0005,dapd_tau_max=0.002,dapd_switch_ratio=0.5,dapd_fast_threshold=0.9,outp_path=evals_results_${model_name}/dapd/${task}-ns0-${length}/results.jsonl \
--output_path evals_results_${model_name}/dapd/${task}-ns0-${length}

# CLAD
CUDA_VISIBLE_DEVICES=${device} accelerate launch eval_llada.py --tasks ${task} --num_fewshot ${num_fewshot} \
--confirm_run_unsafe_code --model llada_dist \
--model_args model_path=${model_path},gen_length=${length},steps=${length},block_length=${block_length},CLAD=True,threshold=0.73,show_speed=True,outp_path=evals_results_${model_name}/CLAD/${task}-ns0-${length}/results.jsonl \
...
CUDA_VISIBLE_DEVICES=${device} accelerate launch eval_llada.py --tasks ${task} --num_fewshot ${num_fewshot} \
--confirm_run_unsafe_code --model llada_dist \
--model_args model_path=${model_path},gen_length=${length},steps=${steps},block_length=${block_length},show_speed=True,dapd=True,dapd_single_block=False,dapd_tau_min=0.0005,dapd_tau_max=0.0010,dapd_switch_ratio=0.5,dapd_fast_threshold=0.9,outp_path=evals_results_${model_name}/dapd/${task}-ns0-${length}/results.jsonl \
