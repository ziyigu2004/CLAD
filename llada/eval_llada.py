...
from generate import generate, generate_with_prefix_cache, generate_with_dual_cache, generate_klass, generate_dapd
from generate_clad import generate_clad
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        dapd_tau_min=0.01,
        dapd_tau_max=0.05,
        dapd_switch_ratio=0.5,
        dapd_fast_threshold=0.9,
        dapd_single_block=False,
        dapd_normalize_mask_graph=False,
        CLAD=False,
        cluster_sym_mode="max",
        ...
        **kwargs,
        ...
        self.CLAD = (
            str(CLAD).lower() == "true"
            if isinstance(CLAD, str)
            else bool(CLAD)
        )
        self.cluster_sym_mode = cluster_sym_mode

        if exact_max_clusters is None or str(exact_max_clusters).lower() in ["none", "null", ""]:
            self.exact_max_clusters = None
        else:
            self.exact_max_clusters = int(exact_max_clusters)
    ...
    def generate_until(self, requests):
        ...
        for batch in tqdm(batched_requests, desc="Generating..."):
            ...
            if self.use_cache:
                ...
                generated_answer, nfe = generate_clad(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length,
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold,
                                        sym_mode=self.cluster_sym_mode)
            elif self.dapd:
                generated_answer, nfe = generate_dapd(
                    self.model,
                    input_ids,
                    steps=self.steps,
                    ...
                    tau_min=self.dapd_tau_min,
                    tau_max=self.dapd_tau_max,
                    switch_ratio=self.dapd_switch_ratio,
                    fast_threshold=self.dapd_fast_threshold,
                    single_block=self.dapd_single_block,
                    normalize_mask_graph=self.dapd_normalize_mask_graph,
                )
            elif self.klass:
                ...
                generated_answer, nfe = generate(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, factor=self.factor, dawn=self.dawn, tau_sink=self.tau_sink, tau_edge=self.tau_edge, tau_induce=self.tau_induce, tau_low=self.tau_low)
...
def _parse_target_doc_args(argv):
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument("--doc_id", type=int, default=None)
    parser.add_argument("--task_id", default=None)
    target_args, remaining = parser.parse_known_args(argv)
    return target_args, remaining


def _install_target_doc_filter(doc_id=None, task_id=None):
    if doc_id is None and task_id is None:
        return

    from lm_eval.api.task import Task
    from lm_eval import utils

    def target_doc_iterator(
        self, *, rank: int = 0, limit=None, world_size: int = 1
    ):
        matched_docs = []
        for original_doc_id, doc in enumerate(self.eval_docs):
            if doc_id is not None and original_doc_id != doc_id:
                continue
            if task_id is not None and str(_get_doc_value(doc, "task_id")) != str(
                task_id
            ):
                continue
            matched_docs.append((original_doc_id, doc))
        ...
        if not matched_docs:
            ...
            raise ValueError(f"No evaluation doc matched {', '.join(filters)}.")
    ...
    Task.doc_iterator = target_doc_iterator


def cli_evaluate_with_target_doc():
    target_args, remaining = _parse_target_doc_args(sys.argv[1:])

    sys.argv = [sys.argv[0]] + remaining
    _install_target_doc_filter(
        doc_id=target_args.doc_id,
        task_id=target_args.task_id,
    )
    cli_evaluate()


if __name__ == "__main__":
    cli_evaluate_with_target_doc()
